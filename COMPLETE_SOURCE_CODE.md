# CivicPulse - Complete Source Code Reference

This file contains the complete source code for all CivicPulse modules. After running `setup_project.py`, place these files in their respective directories.
...
│   ├── main.py              # Entry point (see MAIN.PY section)
│   ├── logic.py             # State machine & decision logic (see LOGIC.PY section)
│   └── services.py          # Google API integrations (see SERVICES.PY section)
├── tests/
│   ├── __init__.py          # Test package
│   ├── test_logic.py        # Logic module tests (see TEST_LOGIC.PY section)
│   └── test_services.py     # Services module tests (see TEST_SERVICES.PY section)
├── .agent/
│   └── skills/
│       ├── deadline_calculation.md      # Deadline skill documentation
│       └── calendar_event_generation.md # Calendar skill documentation
├── setup_project.py         # Repository bootstrap script
...
```bash
# 1. Run setup script to create directories and files
...
# 4. Copy source files into their directories
# (Use your editor or command line to create the files listed below)
...
__description__ = "Interactive election assistant for voter education"
```
...
A smart, dynamic election assistant that guides users through the voting process
using state-based conversation flow and real-time data integration.
...
class CivicPulseAssistant:
    ...
    Manages the conversation flow, state transitions, and coordinates
    interactions with decision logic and external services.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, calendar_id: Optional[str] = None):
        ...
        Args:
            api_key: Google API key for services
    ...
    def process_user_input(self, user_input: str) -> str:
        ...
        Returns:
            Assistant's next message based on state and logic
        """
    ...
    def _handle_polling_location(self, user_input: str) -> str:
        ...
        try:
            ...
            if location_data:
                ...
                message = (
                    f"Perfect! I found your polling location:\n\n"
                    ...
                    f"Now, let's set up election day reminders. "
                    f"What email should I use for calendar invites?"
                )
                return message
            else:
                return (
                    "I couldn't find that location. Could you provide more details? "
                    "(e.g., full address, neighborhood, or nearest intersection)"
                )
    ...
    def _handle_registration_deadline(self, state_input: str) -> str:
        ...
        try:
            ...
            if deadline_info:
                ...
                message = (
                    f"Important dates for {state_input}:\n\n"
                ...
                return (
                    f"I couldn't find information for that state. "
                    f"Please visit your state's election office website or reply with a different state."
                )
        except Exception as e:
    ...
    def _handle_timeline_generation(self, user_input: str) -> str:
        ...
        try:
            ...
            if email or (should_create_event is not None):
                ...
                message = (
                    f"Here's your personalized election timeline:\n\n"
    ...
    def _handle_voting_day_prep(self, user_input: str) -> str:
        ...
        if needs_accommodation:
            message = (
                "Thank you for letting me know. Here are common accommodations available:\n\n"
                "✓ Curbside voting\n"
                "✓ Accessible voting machines\n"
                "✓ Assistance with ballots\n"
                "✓ Extended time for voting\n\n"
                "Contact your local election office at least 5 days before election day "
                "to arrange your specific accommodations.\n\n"
...
[Source code continues in next file due to length...]

See complete_source_code_part2.md for logic.py, services.py, test files, and skill documentation.
